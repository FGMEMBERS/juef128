###############################################################################
# performance.nas by Tatsuhiro Nishioka
...
#
# What is this:
#  The monitor shows the basic aircraft performance such as glide slope, 
#  Mach number, Rate of Climb, Ground speed, Fuel Efficiency, 
#  Estimate Remain Distance, Esitmate Remain Time, and Estimate Cruise Range.
#  You can also measure take-off/landing distance.
#
# How to install:
#  copy 3 nasal files (performance.nas, keyHandler.nas, and dialog.nas) to
#  <Your Aircraft>/Nasal folder. Then add paths for these .nas files into
#  <Your Aircraft>-set.xml file. 
# 
# How to use:
#  You can use performance Monitor by pressing Ctrl-Shift-M.
#  To measure the take-off distance, apply full throttle and fly away.
#  It shows the distance between the starting point and where the aircraft 
#  reaches 10ft above the ground.
#  Landing distance measurement will be available after the aircraft reaches 400ft high.
#  The monitor starts measuring the landing distance when aircraft flies below 50 ft,
#  and it shows the distance between the 50ft point and where aircraft stops.
#  When the aircraft touches down, soft brake will be automatically applied.
#  Ctrl-Shift-M (or Ctrl-Shift-C) reinits the monitor after landing.
#  
# Developer's Guide
...
var initialized = 0;
setlistener("/sim/signals/fdm-initialized", func { settimer(initializeMonitor, 1); });
